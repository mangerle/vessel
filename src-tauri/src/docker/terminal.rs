...
/// 全局终端会话管理器
pub static TERMINAL_SESSIONS: LazyLock<Mutex<HashMap<String, TerminalSession>>> =
...
pub async fn create_container_terminal(
    ...
    } = start_result
    {
        ...
        Err("未能连接到终端流".to_string().into())
    }
}

/// 向终端写入数据
#[tauri::command]
pub async fn write_to_terminal(exec_id: String, data: Vec<u8>) -> AppResult<()> {
    let tx = {
...
/// 关闭容器终端会话
#[tauri::command]
pub async fn close_container_terminal(exec_id: String) -> AppResult<()> {
    let mut sessions = TERMINAL_SESSIONS.lock().await;
    sessions.remove(&exec_id);
    Ok(())
}
